    ...
    try {
        ...
        if (stage === "attachment") {
            ...
            if (realIds.length > 0) {
                ...
                const fileUrls = trainings.map(t => t.fileUrl).filter(Boolean)
            ...
            // 1. Para treinamentos reais já existentes no banco
            if (realIds.length > 0) {
                const trainings = await db.training.findMany({
                    where: { id: { in: realIds }, employeeId },
                    select: { id: true, fileUrl: true }
            ...
            // 2. Para treinamentos virtuais (requisitos da empresa)
        ...
        console.error("DELETE TRAININGS ERROR:", error)
